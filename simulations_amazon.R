...
for (sample_size in sample_sizes){
  ...
  for (i in 1:n_sim) {
    ...
    res <- boot(superpop_psi_random[indices[, i], ], parallel="multicore", ncpus=n_cores,
                estimators_boot,
                R=boot_resample,
                epsilon = 10^-2, maxit=10, verbose = FALSE, seed=seed)
...
for (sample_size in sample_sizes){
  ...
  for (i in 1:n_sim) {
    ...
    res <- boot(superpop_psi_similar[indices[, i], ], parallel="multicore", ncpus=n_cores,
                estimators_boot,
                R=boot_resample,
                epsilon = 10^-2, maxit=10, verbose = FALSE, seed=seed)
