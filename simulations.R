...
# Generate A
A <- ifelse(S==1, r, rbinom(n, 1, psi))

# Create C
C <- as.numeric(r==A)

return(data.frame(X=X, rho=rho, S=S, Pr_Ys1=Pr_Ys1, Pr_Ys0=Pr_Ys0, Y_s1=Y_s1, Y_s0=Y_s0, Y=Y, r=r, A=A, psi=psi, C=C))
}
...
with(dat[1:1500,], plot(Pr_Ys1, Pr_Ys0, pch=16, col=rgb(1,0,0, alpha=.1), xlim=c(0,1), ylim=c(0,1), las=1))
